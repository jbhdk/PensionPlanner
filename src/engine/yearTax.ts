...
/** Årets samlede skat: husstandens egen — personernes lag plus
    aktieindkomstens — plus beholdningernes.

    De tre bærere lægges sammen her og ingen andre steder. `simulate` må ikke
    summere skatter i hånden: det var netop den fejl, ADR-0014 blev skrevet
    om, og den bliver ikke bedre af at blive lavet et niveau længere oppe. Et
    nyt led i en senere etape hører hjemme her, hvor det ikke kan glemmes.
    ...
    `YearResult.tax` — ellers går balanceinvarianten ikke op. */
export function totalYearTax(
  ...
  return totalHouseholdTax(household) + fromHoldings
}
