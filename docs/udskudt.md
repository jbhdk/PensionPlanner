...
## Eksplicit personvalgt aldersforankring på en periode — kun for faste alderstal

**Status:** Besluttet, ikke bygget. [ADR-0050](./adr/0050-foelger-erhvervsophoer-peger-paa-en-navngiven-person-ikke-periodens-udledte-ejer.md) gjorde "Følger erhvervsophør" personvalgt; [ADR-0051](./adr/0051-et-fast-alderstal-kan-pege-eksplicit-paa-en-navngiven-person.md) afgør formen for resten — et **fast alderstal** skal også kunne bære en eksplicit person, og generaliseringen når også `Entry`. Koden er ikke skrevet: `periodOwner` i `validatePlan.ts` udleder stadig alene af figurens struktur (en post har sin egen `owner`, en overførsel bruger afgiverens, en beholdningskildet indbetaling bruger destinationens, jf. [ADR-0028](./adr/0028-det-beholdningskildede-bidrag-maa-krydse-ejerskellet.md)), og der findes intet felt, hvor brugeren vælger den person direkte for et fast tal.

**Hvorfor det betyder noget:** Et fast alderstal på en `PersonAge`-forankret periode er stadig tvetydigt, når Fra og Til har hver sin ejer — der er ingen navngivet person at læse tallet mod. ADR-0050's ejerregel-lempelse rammer derfor kun det tilfælde, hvor *intet* af periodens satte endepunkter er et fast alderstal; er blot ét af dem, spærres `PersonAge`-forankring stadig ved forskellig ejer.

**Prisen ved at lade den ligge:** Mindre end før. De scenarier, ADR-0047 måtte opgive, kan nu aldersforankres, så længe de nøjes med "Følger erhvervsophør" på begge relevante endepunkter. Kun kombinationen af et fast alderstal og en ejerforskel forbliver spærret — og dér, som hidtil, findes kalenderår som vej udenom.

**Hvad der skal bygges, når det tages op:** `PersonAgeBound` udvides med en tredje form, `{ person, age }`, jf. ADR-0051. Rører `periodBounds`/`resolvePersonAgeBound` i `age.ts`, og klemningen i `guardTransferOrContributionAnchor`, som skal behandle `{ person, age }` som utvetydig på linje med `{ person }`. Mindst fire ADR'er bygger på den nuværende "ejeren udledes af strukturen"-antagelse for faste alderstal ([ADR-0016](./adr/0016-indbetalingen-kendes-paa-sin-destination.md), [ADR-0022](./adr/0022-den-skattefri-ordning-toemmes-af-en-overfoersel-ikke-af-en-udbetalingsplan.md), [ADR-0028](./adr/0028-det-beholdningskildede-bidrag-maa-krydse-ejerskellet.md), [ADR-0031](./adr/0031-erhvervsophoersaaret-taeller-med-som-from-og-ikke-med-som-to.md)) og skal genlæses af den, der bygger skiven.
