...
En pensionsordning, der hverken kan oprettes eller modtage en indbetaling mere — aldersopsparingen afløste den ved udgangen af 2012 — og den eneste, hvis `PayoutTaxation` er `Chargeable`. Den forrentes og betaler `PalTax` som de øvrige ordninger, og fra sin `PayoutAge` hæver ejeren af den, som hun vil, mod en afgift af det, der forlader den. Den eneste beholdning i modellen, der kun kan tømmes og aldrig fyldes. Afgiften er ufølsom over for husstandens øvrige indkomst, og det er dét, der gør ordningen værd at planlægge med: den koster det samme i et år, hvor alt andet ville have kostet mere.
_Avoid_: Kapitalforsikring i pensionsøjemed, opsparing i pensionsøjemed, engangspension
...
Hvorfor bufferen er negativ i ét simuleringsår: `Incomplete`, når husstanden har likviditet andetsteds og blot mangler en overførsel, eller `Unsustainable`, når den ikke har. Likviditet andetsteds er præcis de beholdninger, en overførsel kan nå i netop det år — de øvrige frie midler, aktiesparekontoen, og en aldersopsparing eller en kapitalpension, hvis året har passeret dens `PayoutAge` — målt med det beløb, overførslen ville lande og ikke med saldoen. De to halvdele er lige nødvendige: en kapitalpension, hvis afgift æder forskellen, er en beholdning, overførslen kan nå og alligevel ikke lukke hullet med, og `Incomplete` ville da love noget, planen ikke kan indfri. En ratepension tæller ikke med: den kan kun nås af en udbetalingsplan, der binder ti år frem, og det er en anden plan og ikke en manglende overførsel. Et resultat på linje med resten af `YearResult`, ikke en valideringsfejl, og afgjort år for år — samme plan kan være `Incomplete` i de tidlige år og `Unsustainable` senere, jf. ADR-0008.
...
Hvad der sker i skatten, når penge forlader en beholdning: `PersonalIncome` for ratepensionen og livrenten, `TaxFree` for aldersopsparingen, aktiesparekontoen og de to frie varianter, og `Chargeable` for kapitalpensionen. De tre er udtømmende: en udbetaling lander i en indkomstopgørelse, udløser en afgift, eller koster ingenting. En egenskab ved varianten og aldrig ved den enkelte udbetaling, ligesom `Deductibility` på vejen ind. `PersonalIncome` er ikke `EarnedIncome`: en pensionsudbetaling er personlig indkomst uden AM-bidrag og giver hverken beskæftigelsesfradrag eller jobfradrag. Den afgør også, hvad der kan tømme beholdningen: en `PersonalIncome` skal gennem en udbetalingsplan, hvor de to øvrige tømmes af en overførsel.
...
Skatten på en beholdnings årlige afkast, båret af beholdningen selv og trukket af dens saldo. Den passerer aldrig nogen persons indkomst, og den er dermed den tredje bærer ved siden af `TaxAssessment`, som er personens, og `HouseholdTaxAssessment`, som er husstandens. Satsen følger varianten: `PalTax` for `InstalmentPension`, `OldAgeSavings`, `LifeAnnuity` og `CapitalPension`, og aktiesparekontoens egen for `ShareSavingsAccount`. De to frie varianter har ingen — deres afkast beskattes hos personen eller husstanden i stedet.
...
Et tidspunkt angivet ved alder: enten en fast alder, eller en henvisning til `WorkEndAge`. Bruges som start på en beholdnings udbetalingsplan. Sat til erhvervsophør følger punktet `Person.workEndAge` og flytter sig automatisk, uden at planen selv redigeres — erhvervsophørsåret tælles altid med, som det år den første rate falder i. Alderen behøver ikke være et helt år — folkepensionsalderen er 65,5 for én årgang og 72,5 for en anden — og hvilket kalenderår endepunktet rammer, afhænger derfor også af fødselsmåneden.
_Avoid_: Aldersgrænse, tidspunkt

**Personvalgt aldersendepunkt** · `PersonAgeBound`:
Et tidspunkt for en `PersonAge`-forankret periodes endepunkt, i tre former: en fast alder målt på periodens udledte ejer, en fast alder eksplicit målt på en navngiven person, eller en eksplicit henvisning til en navngiven persons erhvervsophør — de to sidste uafhængige af, hvem perioden ellers udledes at tilhøre, jf. [ADR-0050](./docs/adr/0050-foelger-erhvervsophoer-peger-paa-en-navngiven-person-ikke-periodens-udledte-ejer.md) og [ADR-0051](./docs/adr/0051-et-fast-alderstal-kan-pege-eksplicit-paa-en-navngiven-person.md). Sat til erhvervsophør følger den navngivne persons `Person.workEndAge`. Erhvervsophørsåret regnes med som `from` og ikke med som `to`: erhvervsophørsåret er det første år uden arbejde, aldrig det sidste med, jf. [ADR-0031](./docs/adr/0031-erhvervsophoersaaret-taeller-med-som-from-og-ikke-med-som-to.md). En fast alder læses ens i begge roller, uanset om ejeren er udledt eller eksplicit navngivet.
_Avoid_: Aldersendepunkt brugt om den — de to deler form, ikke navn; se `AgeBound`.
...
Det, en fordelingslinje beder om: en procent, et kronebeløb, op til loftet eller resten. En diskrimineret union på sin form, så en form uden et tal ikke kan få et, og en med ikke kan undvære det — formen bærer reglen frem for en validering ved siden af den. Ordet siger fordeling og ikke bare andel, fordi `Andel pr. år` andetsteds i glossaret er en sats og ikke en del af et beløb.
_Avoid_: Andel brugt alene, fordelingsnøgle, vægt, brøk
...
Om ordningen overhovedet kan modtage en `Contribution`. Falsk alene for kapitalpensionen, som har været lukket siden 2012. De to frie varianter står også med falsk, men slås aldrig op: en indbetaling til frie midler er i forvejen en overførsel og afvises som det kategorispørgsmål, den er, med sin egen begrundelse til brugeren. En strukturel umulighed som `UniquePerPerson` og `EmployerAdministered`, afgjort ved indgangen og aldrig i årsresultatet, jf. [ADR-0020](./docs/adr/0020-kan-det-ikke-findes-i-virkeligheden-afvises-det-ved-indgangen.md).
_Avoid_: Lukket ordning, indbetalingsstop, hvilende, passiv

**Overførsel** · `Transfer`:
En dateret flytning af penge fra én af husstandens beholdninger til dens frie midler. Hverken en indtægt eller en udgift, og den bærer ingen skattemæssig egenskab selv: hvad flytningen koster, følger af afgiverens variant, ganske som en indbetalings fradragsret følger af destinationens. Afgiveren skal være en variant, hvis `PayoutTaxation` ikke er `PersonalIncome` — er den `TaxFree`, koster flytningen intet, og er den `Chargeable`, betales der afgift af det, der forlader beholdningen. Skellet er, om loven binder både start, varighed og årligt beløb: gør den det, skal pengene gennem en udbetalingsplan i stedet. Det er også den, der tømmer en aldersopsparing, en aktiesparekonto og en kapitalpension: efter pensionsudbetalingsalderen er de konti, ejeren hæver af som hun vil, og en udbetalingsplan ville påstå en lovregel, der ikke findes, jf. [ADR-0022](./docs/adr/0022-den-skattefri-ordning-toemmes-af-en-overfoersel-ikke-af-en-udbetalingsplan.md). Destinationen er det, der skiller overførslen fra en indbetaling: går pengene ind i en ordning, er det en indbetaling, uanset hvor de kom fra. Perioden kan aldersforankres som en posts, og alderen måles på afgiverbeholdningens ejer — en beholdning har præcis én. Beløbet måles hos afgiveren — det er kroner ud af beholdningen og ikke kroner ind i de frie midler, ganske som en indbetaling fra lønnen måles på bruttolønnen. Det afkortes til afgiverens saldo, og forskellen står på `TransferYear`. Fladen må skrive "udbetaling" om en overførsel ud af en ordning, fordi det er det, den er i virkeligheden; det er en etiket og ikke et begreb. Den bærer et navn som posten og indbetalingen.
...
**Overskudsbånd** · `SurplusBand`:
En af de otte faste dele, `Surplus` består af: indtægtsposter, ydelser, udbetalinger og overførsler ind, som lægger til, og skat, udgiftsposter, indbetalinger og overførsler ud, som trækker fra. Antallet og rækkefølgen ligger fast hele horisonten igennem og uafhængigt af planen — ét bånd pr. post ville give hver plan sin egen graf, og et bånd kunne ikke følges med øjnene fra første til sidste år. Hvad et bånd rummer, afgøres af, hvad der bevæger sig på bufferen, og ikke af hvad ordet lyder som: ATP ligger under indtægtsposterne og ikke under ydelserne, jf. [ADR-0023](./docs/adr/0023-atp-er-en-post-folkepensionen-en-udledning-og-benefit-ingen-type.md), og beholdningsskatten ligger intet sted, fordi den bæres af beholdningen selv og aldrig passerer bufferen. Beløbet er positivt i begge retninger; fortegnet er `Direction`s arbejde, ganske som på en `Entry`. Udledt af årsresultatet og aldrig gemt, ligesom `Surplus` selv.
